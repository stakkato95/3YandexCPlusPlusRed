...
void TestRankingNew() {
    ifstream docs_input("project/docs.txt");
    if (!docs_input.is_open()) {
        return;
    }
    
    ifstream queries_input("project/queries.txt");
    if (!queries_input.is_open()) {
        return;
    }
    ...
    auto startUpdate = system_clock::now();
    srv.UpdateDocumentBase(docs_input);
    auto endUpdate = system_clock::now();
    
    auto startQuery = system_clock::now();
    srv.AddQueriesStream(queries_input, output);
    auto endQuery = system_clock::now();
    
    auto timeUpdate = duration_cast<milliseconds>(endUpdate - startUpdate).count();
    auto timeQuery = duration_cast<milliseconds>(endQuery - startQuery).count();
    
    cout << timeUpdate << " update" << endl;
    cout << timeQuery << " query" << endl;
    cout << timeUpdate + timeQuery << " total" << endl;
}
